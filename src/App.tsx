...
import SupplementsAndVitamins from "./components/SupplementsAndVitamins";
...
function AppContent() {
  ...
  const [programBuilderView, setProgramBuilderView] = useState<'builder' | 'analytics' | 'library'>('builder');
  ...
  const [selectedWeekIdx, setSelectedWeekIdx] = useState<number | undefined>(undefined);
    ...
    if (savedUser) {
      try {
        ...
        if (parsedUser && parsedUser.email) {
          // Force admin role for specific emails immediately in state
          if (parsedUser.email === 'swolecode@gmail.com' || parsedUser.email === 'kromefitness@gmail.com') {
            parsedUser.role = 'admin';
          }
          
          setUser(parsedUser);
          
          // Refresh from API to get latest data (including role changes)
          ...
          if (userId) {
            fetch(`/api/users/${userId}`)
              .then(async res => {
                if (!res.ok) {
                  if (res.status === 404) {
                    // User exists in local storage but not in DB. Register them.
                    console.log("User not found in DB, registering from local storage data...");
  ...
  const MobileNav = () => {
    const navItems = [
      { id: 'messages', icon: <NotificationIcon userId={user?.id} onOpenChat={() => setShowChat(true)} onOpenAdminChat={() => { setAdminInitialTab('chat'); navigateTo('admin'); safeStorage.setItem('krome_admin_active_tab', 'chat'); }} isAdmin={user?.role === 'admin' || user?.role === 'coach'} unreadCount={unreadCount} standalone={false} />, label: 'Messages' },
      { id: 'programCatalog', icon: <Zap className="w-5 h-5" />, label: 'Programs' },
      { id: 'shop', icon: <ShoppingBag className="w-5 h-5" />, label: 'Shop' },
      ...(user?.role === 'admin' || user?.role === 'coach' ? [{ id: 'admin', icon: <Shield className="w-5 h-5" />, label: user?.role === 'admin' ? 'Admin' : 'Coach' }] : []),
      { id: 'profile', icon: <User className="w-5 h-5" />, label: 'Profile' },
      { id: 'more', icon: <Menu className="w-5 h-5" />, label: 'More' },
    ];

    const topNavViews = [
      'myPrograms', 'programCalendar', 'programCatalog', 'fitnessOverview', 
      'bodyMetrics', 'bodyComposition', 'videoAnalysis', 'nutritionDashboard', 
      'performanceMacroNutrients', 'workoutTracker', 'profile'
    ];
    const isTop = topNavViews.includes(currentView) || currentView === 'auth';

    return (
      <div className={`md:!hidden fixed ${isTop ? 'top-0 border-b pt-[calc(12px+var(--safe-area-top))]' : 'bottom-0 border-t pb-[calc(12px+env(safe-area-inset-bottom,0px))]'} left-0 w-full bg-black border-white/20 px-4 py-3 flex justify-between items-center z-[100] shadow-[0_-10px_30px_rgba(0,0,0,0.8)]`}>
        {navItems.map((item) => (
          <button
            key={item.id}
            onClick={() => {
  ...
  // Removed forced onboarding redirect to allow seamless navigation to other views.
  // Users will still see the onboarding banner on their profile if incomplete.
  useEffect(() => {
    if (user) {
  ...
  useEffect(() => {
    ...
    // Handle shared view/userId
    const viewParam = query.get('view') as View;
    const userIdParam = query.get('userId');
    if (viewParam && viewParam !== 'home') {
      navigateTo(viewParam, userIdParam || undefined);
      window.history.replaceState({}, document.title, window.location.pathname);
    }
  }, []);
  ...
  const isOwnProfile = !targetUserId || targetUserId === user?.id.toString() || targetUserId === user?.uid;

  const handleLoginSuccess = (userData: any) => {
    const hasGoal = !!userData.fitness_goal;
    const hasParq = userData.parq_completed === 1 || userData.parq_completed === true;
  ...
  return (
    <NotificationProvider>
      <ErrorBoundary>
        <div className="min-h-screen bg-black/40 backdrop-blur-xl overflow-x-hidden">
          <Toaster position="top-center" expand={true} richColors />
        {/* Navbar */}
      <nav 
        role="navigation" 
        aria-label="Main navigation"
        className={`${currentView === 'home' ? 'block' : 'hidden'} md:block fixed top-0 w-full z-[1000] transition-all duration-300 ${
          currentView === 'auth' ? '!hidden' :
          (isScrolled || currentView !== 'home' || mobileMenuOpen ? "bg-black/95 backdrop-blur-xl border-b border-white/10 shadow-2xl shadow-black/50" : "bg-transparent")
        }`}
        style={{ 
      ...
      >
        <div className={`max-w-7xl mx-auto px-6 flex justify-between items-center transition-all duration-300 ${isScrolled || currentView !== 'home' || mobileMenuOpen ? "py-2" : "py-4"}`}>
            ...
            >
              <span className="text-xl font-black tracking-tighter uppercase italic text-gold">
          ...
          <div className="hidden md:flex items-center gap-8 text-sm font-bold uppercase tracking-widest">
            <a href="#" onClick={(e) => { e.preventDefault(); navigateTo('home'); }} className="text-krome hover:text-gold transition-colors !outline-none hover:krome-outline px-2 py-1 rounded-lg">Home</a>
            ...
            >
              <button 
                className="flex items-center gap-1 text-krome hover:text-gold transition-colors uppercase tracking-widest !outline-none hover:krome-outline px-2 py-1 rounded-lg"
            ...
            <a 
              ...
              className="text-krome hover:text-gold transition-colors !outline-none hover:krome-outline px-2 py-1 rounded-lg"
            >
              Contact KSP
            </a>
            ...
            {(user?.role === 'admin' || user?.role === 'coach') && (
              ...
              >
                <Shield className="w-4 h-4" /> {user?.role === 'admin' ? 'Admin' : 'Coach Dashboard'}
              </button>
            )}
            ...
            {user ? (
              ...
              >
                ...
                <span className="text-xs font-bold text-[#b2d8d8] transition-colors">{user.username}</span>
              </button>
            ) : (
              <button 
                onClick={() => { setAuthMode('login'); navigateTo('auth'); }}
                className="btn-gold !py-2 !px-6 !text-xs !outline-none krome-outline"
              >
                Login
              </button>
            )}
      ...
      <AnimatePresence>
          ...
          >
            <div className="flex justify-between items-center mb-4">
              <div className="flex items-center">
                <span className="text-lg font-black tracking-tighter uppercase italic text-gold">KROME <span className="text-gold">Sports</span></span>
              </div>
            ...
            <a href="#" onClick={(e) => { e.preventDefault(); navigateTo('home'); setMobileMenuOpen(false); }} className="text-krome !outline-none py-2 border-b border-white/5">Home</a>
            <a href="#" onClick={(e) => { e.preventDefault(); navigateTo('contact'); setMobileMenuOpen(false); }} className="text-krome !outline-none py-2 border-b border-white/5">Contact</a>
            ...
            <div className="mt-4 border-t border-white/5 pt-4 flex flex-col gap-4">
              ...
              {(user?.role === 'admin' || user?.role === 'coach') && (
                <button onClick={() => { navigateTo('admin'); setMobileMenuOpen(false); }} className="text-gold !outline-none py-2">{user?.role === 'admin' ? 'Admin Dashboard' : 'Coach Dashboard'}</button>
              )}
              ...
              {user ? (
                ...
                <button 
                  ...
                  className="btn-gold mx-auto !outline-none krome-outline"
                >
                  Login
                </button>
              )}
        ...
        {showChat && user && (
          <div className="fixed bottom-6 right-6 z-[110] w-full max-w-sm px-6 md:px-0">
            <motion.div
        ...
        {currentView !== 'home' && currentView !== 'auth' && <MobileNav />}
      ...
      <AnimatePresence mode="wait">
            ...
            <section className="relative h-screen flex items-center justify-center overflow-hidden">
              ...
              <div className="relative z-10 max-w-5xl mx-auto px-6 text-center">
                ...
                >
                  ...
                  <h1 className="text-5xl md:text-9xl font-black uppercase italic leading-[0.9] tracking-tighter mb-6 px-4">
                    <span className="text-transparent bg-clip-text bg-gradient-to-r from-gold to-accent">Unlock Your</span> <br />
              ...
              <div className="max-w-7xl mx-auto px-6 relative z-10">
                <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-6">
                  <div>
                    ...
                    <h3 className="text-4xl md:text-6xl font-black uppercase italic leading-[1.1] px-4">
                      <span className="text-transparent bg-clip-text bg-gradient-to-r from-gold to-accent">Performance</span> <br />
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                  {programs.map((program, idx) => (
                    <motion.div
                      ...
                      className="group relative bg-zinc-900 rounded-2xl overflow-hidden border border-[#b2d8d8]/30 cursor-pointer hover:border-[#b2d8d8] transition-colors"
                    >
                      ...
                      <div className="p-6">
                        ...
                        <h4 className="text-lg font-bold mb-2 uppercase italic text-[#b2d8d8]">{program.title}</h4>
              ...
              <div className="max-w-7xl mx-auto px-6 relative z-10">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
                  <motion.div
                    ...
                    viewport={{ once: true }}
                  >
                    <h2 className="text-5xl md:text-7xl font-black uppercase italic leading-[1.1] mb-8 px-4">
                      <span className="text-transparent bg-clip-text bg-gradient-to-r from-gold to-accent">Master Every</span> <br />
        ...
        {currentView === 'programCatalog' && (
          <ViewWrapper viewKey="programCatalog">
            <ProgramCatalog 
              key="programCatalog"
        ...
        {currentView === 'myPrograms' && (
          <ViewWrapper viewKey="myPrograms">
            <ProgramCatalog 
              key="myPrograms"
              userId={user?.id || 'guest'}
              isAdmin={user?.role === 'admin' || user?.role === 'coach'}
              type="myPrograms"
              onBack={goBack}
              onBrowseCatalog={() => navigateTo('programCatalog')}
              onSelectProgram={(program, locked) => {
                if (locked && !user) {
                  setAuthMode('login');
                  navigateTo('auth');
        ...
        {currentView === 'breakPrograms' && (
          <ViewWrapper viewKey="breakPrograms">
            <ProgramCatalog 
              key="breakPrograms"
              userId={user?.id || 'guest'}
              isAdmin={user?.role === 'admin' || user?.role === 'coach'}
        ...
        {currentView === 'movementPrograms' && (
          <ViewWrapper viewKey="movementPrograms">
            <ProgramCatalog 
        ...
        {currentView === 'programIntro' && (
          <ViewWrapper viewKey="programIntro">
            <ProgramIntro 
              key="programIntro" 
              onBack={goBack} 
              onSelectPhase={(num) => {
        ...
        {currentView === 'movementLanding' && (
          <ViewWrapper viewKey="movementLanding">
            <MovementLanding 
              key="movementLanding"
              onBack={goBack}
              onStartProgram={() => {
        ...
        {currentView === 'nutritionLanding' && (
          <ViewWrapper viewKey="nutritionLanding">
            <NutritionLanding 
        ...
        {currentView === 'breakProgramsLanding' && (
          <ViewWrapper viewKey="breakProgramsLanding">
        ...
        {currentView === 'specializedLanding' && (
          <ViewWrapper viewKey="specializedLanding">
        ...
        {currentView === 'specialized' && (
          <ViewWrapper viewKey="specialized">
            <SpecializedProgram 
        ...
        {currentView === 'fitnessGoal' && user && (
          <ViewWrapper viewKey="fitnessGoal">
        ...
        {currentView === 'onboarding' && user && (
          <ViewWrapper viewKey="onboarding">
            <OnboardingFlow 
              user={user}
              onComplete={() => {
        ...
        {currentView === 'profile' && user && (
          <ViewWrapper viewKey="profile">
            <Profile 
              ...
              onNavigate={(view) => navigateTo(view as View)}
              onProgramSelect={(programId, phaseIdx, weekIdx, workoutId) => {
                console.log(`[Profile] Selected program: ${programId}`);
                setSelectedProgramId(programId);
                setSelectedPhaseIdx(phaseIdx ?? 0);
                setSelectedWeekIdx(weekIdx ?? 0);
                setSelectedWorkoutId(workoutId || null);
                navigateTo('programViewer');
              }}
              initialTab={user.parq_completed === 0 ? 'parq' : 'dashboard'}
            />
          </ViewWrapper>
        )}

        {currentView === 'workoutTracker' && user && (
          <div className="px-6 max-w-4xl mx-auto" style={{ paddingTop: 'calc(100px + var(--safe-area-top))' }}>
        ...
        {currentView === 'progressTracker' && user && (
          <div className="px-6 max-w-4xl mx-auto" style={{ paddingTop: 'calc(100px + var(--safe-area-top))' }}>
            ...
            <ProgressTracker 
              userId={targetUserId ? parseInt(targetUserId) : user.id} 
              isOwnProfile={isOwnProfile}
              onBack={goBack}
            />
          </div>
        ...
        {currentView === 'nutritionDashboard' && (
          user ? (
            <div className="px-6 max-w-4xl mx-auto" style={{ paddingTop: 'calc(100px + var(--safe-area-top))' }}>
              <div className="flex justify-between items-center mb-8">
                <button 
        ...
        {currentView === 'supplementsAndVitamins' && (
          user ? (
            <div className="px-6 max-w-4xl mx-auto" style={{ paddingTop: 'calc(100px + var(--safe-area-top))' }}>
              <div className="flex justify-between items-center mb-8">
                <button 
              ...
              <SupplementsAndVitamins 
                ...
                setData={setBodyMetricsData} 
                isEditing={false}
                getSupplementRecommendation={() => ''}
                generateDefaultSupplements={() => {}}
              />
            </div>
          ) : (
            <Auth key="auth" initialMode="login" onBack={goBack} onLoginSuccess={handleLoginSuccess} />
          ...
          user ? (
            <div className="px-6 max-w-7xl mx-auto" style={{ paddingTop: 'calc(100px + var(--safe-area-top))' }}>
              <ProgramViewer 
                ...
                initialPhaseIdx={selectedPhaseIdx}
                initialWeekIdx={selectedWeekIdx}
      ...
      {/* Footer */}
      <footer role="contentinfo" className="py-20 bg-black border-t border-white/10 relative z-10">
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex flex-col md:flex-row justify-between items-start gap-12 mb-16">
            <div className="max-w-sm">
              ...
              <div className="flex gap-4">
                <a href="https://instagram.com/ksp_2407" target="_blank" rel="noopener noreferrer" aria-label="Instagram" className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center hover:bg-gold hover:text-black transition-all border border-[#b2d8d8] text-[#b2d8d8] hover:text-black">
                  <Instagram className="w-5 h-5" aria-hidden="true" />
                </a>
                <a href="https://youtube.com/@kromesports" target="_blank" rel="noopener noreferrer" aria-label="YouTube" className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center hover:bg-gold hover:text-black transition-all border border-[#b2d8d8] text-[#b2d8d8] hover:text-black">
                  <Youtube className="w-5 h-5" aria-hidden="true" />
                </a>
                <a href="https://facebook.com/kromesports" target="_blank" rel="noopener noreferrer" aria-label="Facebook" className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center hover:bg-gold hover:text-black transition-all border border-[#b2d8d8] text-[#b2d8d8] hover:text-black">
            ...
            <div className="grid grid-cols-1 gap-16">
              <div>
                <h5 className="font-bold uppercase tracking-widest text-xs text-krome mb-6">Quick Links</h5>
                <ul className="flex flex-row gap-6 text-sm font-medium text-krome">
          ...
          <div className="pt-8 border-t border-white/10 bg-black flex flex-col md:flex-row justify-between items-center gap-8 text-[10px] text-white font-bold uppercase tracking-[0.2em]">
            ...
            <p className="text-[#b2d8d8]">Designed by SwoleCode Small Business Solutions</p>
          </div>
        </div>
      </footer>
      {/* Chatbot */}
