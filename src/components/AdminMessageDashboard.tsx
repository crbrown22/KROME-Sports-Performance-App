...
export default function AdminMessageDashboard({ adminId }: { adminId: number }) {
  ...
  const fetchUnread = async () => {
    try {
      const response = await fetch(`${window.location.origin}/api/messages/unread?userId=${adminId}`);
      if (response.ok) {
  ...
  useEffect(() => {
    fetch('/api/users')
      .then(res => res.json())
      .then(data => setUsers(data.filter((u: UserRecord) => u.id !== adminId)));
    ...
    try {
      ...
      if (response.ok) {
        ...
        const unreadIds = data
          .filter((m: any) => m.receiver_id === adminId && m.is_read === 0)
          .map((m: any) => m.id);
  ...
  const sendMessage = async (text: string) => {
    ...
    try {
      await fetch(`${window.location.origin}/api/messages`, {
        ...
        body: JSON.stringify({ sender_id: adminId, receiver_id: selectedUser.id, message: text })
      });
      setNewMessage('');
      fetchMessages(selectedUser.id);
    } catch (err) {
        ...
        {selectedUser ? (
          <>
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-6 custom-scrollbar">
              {messages.map((msg) => (
                <div key={msg.id} className={`flex gap-4 ${msg.sender_id === adminId ? 'flex-row-reverse' : ''}`}>
                  <div className={`max-w-[70%] p-4 rounded-3xl text-sm shadow-xl relative group ${msg.sender_id === adminId ? 'bg-gold text-black rounded-tr-none font-medium' : 'bg-zinc-800/80 border border-white/5 text-white/90 rounded-tl-none'}`}>
                    <div className="leading-relaxed">
                      {renderMessageContent(msg.message)}
                    </div>
                    <p className={`text-[9px] mt-2 font-bold uppercase tracking-widest opacity-40 ${msg.sender_id === adminId ? 'text-black' : 'text-white'}`}>
                      {msg.sender_id === adminId ? 'Sent' : 'Received'} • {new Date(msg.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                    </p>
                  </div>
                </div>
              ))}
